...
def cvsco(user, module):
    """Checks out module in cvs as user"""
    print "*** Checking out %s from RPM Fusion CVS (free repo) as %s:" % (module, user or 'anonymous')
    environment = os.environ
    if user:
      environment['CVSROOT'] = ':ext:%s@cvs.rpmfusion.org:/cvs/free/' % user
    else:
      environment['CVSROOT'] = ':pserver:anonymous@cvs.rpmfusion.org:/cvs/free/'
...
def usage(error=None):
    if error:
        ...
        print 'rpmfusion-free-cvs: Checkout modules from RPM Fusion CVS (free repo)'
    print """Usage: rpmfusion-free-cvs [module list]
  -h, --help: Show this help message.
  -a, --anonymous: Use anonymous CVS.

Example: rpmfusion-free-cvs comps owners ffmpeg"""
